...
fn main() {
    ...
    // Waldlust: packer 는 더 이상 설치기가 아니다(Defender 오탐 회피 위해 자동설치 제거).
    // 우리 배포 파일명(waldlust-x.x.x-x86_64.exe)은 "...install.exe"로 안 끝나므로
    // click_setup 은 항상 false → 더블클릭하면 그냥 포터블로 앱만 실행된다. 실제 설치는
    // 별도 인스톨러(res/inno/waldlust.iss)가 wald-remote.exe --after-install 로 수행한다.
